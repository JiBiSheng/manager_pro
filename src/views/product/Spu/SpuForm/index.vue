  ...
  <div>
    <el-form ref="form" label-width="80px" :model="spu">
      ...
      <el-form-item label="销售属性">
        <el-select :placeholder="`还有${unSelectSaleAttr.length}个没有选择`" v-model="attrIdAndAttrName">
          <el-option :label="unselect.name" v-for="(unselect,index) in unSelectSaleAttr" :key="unselect.id" :value="`${unselect.id}:${unselect.name}`"></el-option>
        </el-select>
        ...
        <el-table border style="width: 100%" :data="spu.spuSaleAttrList">
          ...
          <el-table-column prop="prop" label="属性值列表" width="width">
            <template slot-scope="{ row, $index }">
              ...
              <el-tag :key="tag.id" v-for="(tag,index) in row.spuSaleAttrValueList" closable :disable-transitions="false"  @close="row.spuSaleAttrValueList.splice(index,1)" >{{ tag.saleAttrName }}</el-tag>
